...
const SecureChatBox = ({ currentUser}) => {
...
useEffect(()=>{
  console.log("Arrieved Encrypted message----",arrivalMessage?.encryptedMessage)
  const receiverId = chat?.members?.find(member=> member !==authUser._id)
  const decrypt =async()=>{
    let newdecryptedMessage = await signalProtocolManagerUser?.decryptMessageAsync(receiverId, arrivalMessage?.encryptedMessage)
    console.log("After decryption------",newdecryptedMessage)
    
    arrivalMessage &&
    chat?.members.includes(arrivalMessage.sender)&&
    setMessages((prev)=> [...prev, {sender:arrivalMessage.sender, text:newdecryptedMessage,createdAt:Date.now()}]);
...
getkeyBundle();
  let encryptedMessage = await signalProtocolManagerUser.encryptMessageAsync(receiverId, newMessage,keyBundles);
  ...
  try{
      ...
      setMessages([...messages, {text:"aaaa",}])
      setNewMessage("")
      const receiverId = chat.members.find(member=> member !==authUser._id)
      socket.emit("send-secureMessage",{
          conversationId:chat._id,
          senderId: authUser._id,
          receiverId,
          senderName: authUser.username,
