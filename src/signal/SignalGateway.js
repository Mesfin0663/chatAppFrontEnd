...
import { keyBundle } from './prekeyBundle';
...
export class SignalServerStore {
    ...
    registerNewPreKeyBundle(userId, preKeyBundle) {
        ...
        const setkeyBundle = async () => {
            try {
                const res = await axios.post("/api/keybundle/create", { userId: userId, keyBundle: storageBundle });
     ...
     */
    getPreKeyBundle(userId,keyBundles) {
...
class SignalProtocolManager {
     ...
     */
    async encryptMessageAsync(remoteUserId, message,keyBundles) {
        ...
        if (sessionCipher == null) {
            ...
            console.log("remote user address",address)
            // Instantiate a SessionBuilder for a remote recipientId + deviceId tuple.
            var sessionBuilder = new libsignal.SessionBuilder(this.store, address);
            console.log(" remote user session builder",sessionBuilder)
            var remoteUserPreKey = this.signalServerStore.getPreKeyBundle(remoteUserId,keyBundles);
            ...
            console.log("sessionCipher", console.log(sessionCipher));
    ...
    async _generateIdentityAsync() {
        ...
        console.log("Generates a new identity for the local user.")
        console.log('identityKey', results[0])
        console.log('registrationId', results[1])
